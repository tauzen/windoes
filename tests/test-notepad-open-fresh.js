...
async function runTests() {
  ...
  try {
    ...
    assert(
      seededState.text === 'Previously opened document text',
      'sanity: seeded explicit file-open content is shown'
    );
    assert(
      seededState.filePath === '/C:/My Documents/previous.txt',
      'sanity: seeded explicit file-open path is tracked'
    );
    assert(
      seededState.title === 'previous.txt - Notepad',
      'sanity: seeded explicit file-open title is shown'
    );
    ...
    assert(
      reopenedState.title === 'Untitled - Notepad',
      `plain open resets title to untitled (got: ${reopenedState.title})`
    );
    ...
    assert(
      explicitState.title === 'open-specific.txt - Notepad',
      `explicit file-open sets file title (got: ${explicitState.title})`
    );
  } finally {
    await browser.close();
    server.close();
