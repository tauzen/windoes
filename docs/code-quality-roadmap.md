...
- [ ] **A1 / A7 / D7** Eliminate `desktop.jsx` + `fs-explorer.jsx`'s module-level DOM refs. `fs-explorer` becomes a `<MyComputer>` component using hooks (`useVirtualFs`, `useNavigationHistory`). _Progress: removed cached module-level DOM element refs in `fs-explorer.jsx`, added remount-safety coverage for desktop icon interactions, and refactored `desktop.jsx` to delegated handlers + on-demand DOM lookups (no cached icon-node arrays / per-icon listener wiring). Remaining: full hook-based `<MyComputer>` componentization and React-owned desktop icon rendering._
