...
- [ ] **A1 / A7 / D7** Eliminate `desktop.jsx` + `fs-explorer.jsx`'s module-level DOM refs. `fs-explorer` becomes a `<MyComputer>` component using hooks (`useVirtualFs`, `useNavigationHistory`). _Progress: removed cached module-level DOM element refs in `fs-explorer.jsx`, added remount-safety coverage for desktop icon interactions, refactored `desktop.jsx` to delegated handlers + on-demand DOM lookups (no cached icon-node arrays / per-icon listener wiring), reset explorer navigation session state on My Computer open (reopen starts clean with Back disabled), extracted explorer navigation/history logic into `explorer-navigation.mjs` with focused unit coverage (`tests/node/explorer-navigation.test.js`), and introduced a React-owned `<MyComputerToolbar>` that consumes navigation snapshot/subscription state (component marker + integration coverage in Test 20) as a seam toward `useNavigationHistory` hook adoption. Remaining: full hook-based `<MyComputer>` componentization and React-owned desktop icon rendering._
