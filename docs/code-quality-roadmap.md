...
| B11 | `navigate()` in ie-window uses `window.stop()` on Stop button — stops the *host* page, not just the iframe. Should call `frame.contentWindow.stop()` (or reload about:blank). | `ie-window.jsx:150-153` |
...
### Phase 0 — Guardrails (½–1 day, low risk)

Set up the tooling we need before changing code.

- [ ] **H2 / H3** Add Prettier + ESLint (flat config) with a minimal ruleset (no-unused-vars, react/jsx-key, no-implicit-globals, consistent-return).
- [ ] **G1** Replace `tests/run-tests.js` with Vitest _or_ `node --test` and a single `npm test` entry. Keep the Playwright integration tests but run them through the new runner.
- [ ] **G2** Add a pure reducer unit test suite for `app-state.js` (every `case` gets at least one fixture).
- [ ] **H3** Add husky + lint-staged running Prettier on staged files.
- [ ] **H4** Document the reducer action shapes with JSDoc `@typedef` blocks.
...
- [ ] **A4** Collapse `WindowManager._stack` / `_windows[id].isOpen` / `.isMaximized` into reducer actions (`WINDOW_OPEN`, `WINDOW_CLOSE`, `WINDOW_MINIMIZE`, `WINDOW_RESTORE`, `WINDOW_MAXIMIZE_TOGGLE`, `WINDOW_FOCUS`). Keep `WindowManager` as a view layer that *reads* state and applies DOM side effects.
