...
async function runTests() {
  ...
  try {
    ...
    assert(
      afterWinamp.length === 1,
      `Exactly 1 window opens (got ${afterWinamp.length}: ${afterWinamp})`
    );
    ...
    assert(
      afterCloseWinamp.length === 0,
      `No windows remain after closing Winamp (got ${afterCloseWinamp.length}: ${afterCloseWinamp})`
    );
    ...
    assert(
      afterMine.length === 1,
      `Exactly 1 window opens (got ${afterMine.length}: ${afterMine})`
    );
    ...
    assert(
      afterCloseMine.length === 0,
      `No windows remain after closing Minesweeper (got ${afterCloseMine.length}: ${afterCloseMine})`
    );
    ...
    assert(
      bothOpen.length === 2,
      `Exactly 2 windows when both are open (got ${bothOpen.length}: ${bothOpen})`
    );
      ...
      window.WIN_ME_SIMULATOR_CONFIG = {
        experimentApps: [
          { id: 'iconWinamp', title: 'Winamp', url: './applications/winamp-player/index.html' },
          {
            id: 'iconMinesweeper',
            title: 'Minesweeper',
            url: './applications/minesweeper/index.html',
          },
        ],
      };
    });
    ...
    assert(
      guardedWinamp.length === 1,
      `Defensive guard: only 1 window opens for Winamp (got ${guardedWinamp.length}: ${guardedWinamp})`
    );
    assert(
      !guardedWinamp.includes('appWindow'),
      'Defensive guard: generic appWindow NOT opened for Winamp'
    );
    ...
    assert(
      guardedMine.length === 1,
      `Defensive guard: only 1 window opens for Minesweeper (got ${guardedMine.length}: ${guardedMine})`
    );
    assert(
      !guardedMine.includes('appWindow'),
      'Defensive guard: generic appWindow NOT opened for Minesweeper'
    );
