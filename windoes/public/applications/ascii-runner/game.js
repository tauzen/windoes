...
function initBackground() {
  ...
  for (let i = 0; i < 60; i++) {
    background.stars.push({
      ...
      color: [COLORS.neonCyan, COLORS.neonPink, COLORS.neonGreen, COLORS.textPrimary][
  ...
  for (let i = 0; i < ASCII_MOUNTAINS.length; i++) {
    ...
    for (let repeat = 0; repeat < 4; repeat++) {
      ...
      if (x < BASE_WIDTH + 100 && x > -pattern.length * CHAR_WIDTH) {
        ...
        const color =
          colorIndex < 0.3 ? COLORS.neonCyan : colorIndex < 0.6 ? COLORS.bgLight : COLORS.textMuted;
...
function createDeathParticles() {
  for (let i = 0; i < 20; i++) {
    deathParticles.push({
      ...
      color: [COLORS.accentPrimary, COLORS.neonPink, COLORS.neonCyan][
