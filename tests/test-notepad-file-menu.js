...
async function runTests() {
  ...
  try {
    ...
    const fileMenuTriggerStateBeforeOpen = await page.evaluate(() => {
      const trigger = document.getElementById('notepadFileMenu');
      return {
        hasPopup: trigger?.getAttribute('aria-haspopup') === 'menu',
        controls: trigger?.getAttribute('aria-controls') === 'notepadFileDropdown',
        expanded: trigger?.getAttribute('aria-expanded') === 'true',
      };
    });

    assert(
      fileMenuTriggerStateBeforeOpen.hasPopup,
      'Notepad File trigger exposes aria-haspopup=menu'
    );
    assert(fileMenuTriggerStateBeforeOpen.controls, 'Notepad File trigger exposes aria-controls');
    assert(
      !fileMenuTriggerStateBeforeOpen.expanded,
      'Notepad File trigger aria-expanded is false before open'
    );
    ...
    const menuState = await page.evaluate(() => {
      ...
      const labels = menu
        ...
        : [];
      const notepadState =
        window.WindoesApp && WindoesApp.state && WindoesApp.state.get
    ...
    assert(
      menuState.stateMenuOpen,
      'Notepad File dropdown open state is reflected in reducer state'
    );
    assert(menuState.triggerExpanded, 'Notepad File trigger aria-expanded is true while open');
    ...
    assert(
      saveAsResult.newContent === textAfterSaveAs,
      'Save As writes latest content to new file'
    );
    ...
    assert(
      saveAsResult.title === saveAsResult.expectedTitle,
      'Save As updates title to new file name'
    );
    ...
    assert(
      newState.title === 'Untitled - Notepad',
      `New action resets title (got: ${newState.title})`
    );
    ...
    const isHidden = await page.evaluate(() =>
      document.getElementById('notepadWindow').classList.contains('hidden')
    );
